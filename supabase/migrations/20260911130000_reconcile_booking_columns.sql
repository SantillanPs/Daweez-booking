-- ==========================================
-- Reconcile the booking columns the live database was missing
-- Supabase PostgreSQL Migration
--
-- The deployed project's migration history stopped at 20260818093046, so
-- 20260820100000 / 20260822090000 / 20260823090000 were applied by hand and
-- three columns the app already sends never made it to the live table:
-- breakfast_records, reference_number and registered_on. Everything the app
-- wrote to them was silently dropped (per-day breakfast during a stay, and the
-- original paper-log number from "Log old booking").
--
-- This migration adds those columns and RE-CREATES both booking RPCs from the
-- full live definition, so they now carry every column the app sends —
-- including payment_plan from 20260911120000_add_payment_plan.sql. It is
-- idempotent and safe to re-run; the definitions below are exactly what is
-- deployed.
-- ==========================================

-- 1. Missing booking columns
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS breakfast_records jsonb DEFAULT NULL;
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS reference_number text DEFAULT NULL;
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS registered_on date DEFAULT NULL;

-- 2. book_booking / update_booking — full live definitions
