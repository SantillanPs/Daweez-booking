-- ==========================================
-- Fix booking RPC id type (varchar, not uuid)
-- Supabase PostgreSQL Migration
--
-- bookings.id is VARCHAR and holds legacy ids like 'imported-...' that are
-- NOT valid UUIDs. update_booking compared b.id (varchar) to v_id (uuid),
-- which crashed with "operator does not exist: character varying <> uuid".
-- Fix: treat the id as TEXT in every booking RPC so legacy and generated ids
-- both work. gen_random_uuid() is cast to ::text for the fallback path.
-- ==========================================
...
-- 3. confirm_booking — id as text (bookings.id is varchar)
...
-- 4. delete_booking — id as text (bookings.id is varchar)
