...
- **Data Operations**:
  ...
  - [syncEngine.ts](file:///c:/Users/dev4s/Documents/Programming/Daweez/src/utils/syncEngine.ts) is a barrel re-export. Actual logic lives in domain modules under `src/utils/`:
    ...
    - [statement.ts](file:///c:/Users/dev4s/Documents/Programming/Daweez/src/utils/statement.ts) — `buildStatement`: turns a booking (and any related bookings sharing an invoice number) into structured line items + payment totals for the printable Guest Billing Statement (stay rows, one row per per-day breakfast served, extra linen/rental equipment, venue excess hours, event add-ons, early/late check-in-out, staff discount), reusing `calculatePricing` (and taking early check-in hours through `chargeableEarlyHours`, so the printed bill only carries them once the stay is checked out — the screen and the paper must never disagree). `dateIssued` carries the bill date + time. Its `amountDue` is what the guest hands over at this point — the agreed figure while nothing is paid (**Deposit**: half the stay · **Full pay**: the whole stay · **Custom**: the amount the desk typed), otherwise what is left (`amountToPayNow`, never the whole stay) — with `balanceAfter` for the rest and `paymentPlan` carrying the guest's own choice (`'deposit' | 'full' | 'custom' | ''`) through to the printed page, so the statement can name a custom figure instead of printing the bare word. Its `tabLinesByBooking` input (k69) prints the guest's food and bar lines under the room as `Restaurant & bar · <item>` rows and adds their total to the Sub-Total, so the paper agrees with the balance the screen shows; the same total is passed to `amountToPayNow` so the printed deposit stays worked out from the stay alone.


    - [shortStay.ts](file:///c:/Users/dev4s/Documents/Programming/Daweez/src/utils/shortStay.ts) — the short-stay clock, the ONE place that works out when a room sold for hours is free again: `stayHoursOf`, `shortStayEnd` (the recorded arrival + the hours bought, `null` until the desk has checked the guest in, because the clock starts at that press), `clockLabel` (`1:12 PM`) and `shortStayLine` (`3 hours from arrival` / `3 hours · ends 1:12 PM`) for the printed bill. The calendar, the booking panel and the statement all read it — see the calendar doc for the pop-out rule.

    - [hotelPolicy.ts](file:///c:/Users/dev4s/Documents/Programming/Daweez/src/utils/hotelPolicy.ts) — `HOTEL_POLICY`, the hotel's Pension Policies copied **verbatim** from the paper registration form (the owner photographed it and ruled: word for word, on every printed bill). It exists because the app had been printing its own paraphrase, which had dropped the ₱1,000 smoking penalty and its Tandag City ordinance, the rule that key cards must be left at Reception, and the ₱500 lost-key-card charge, and had invented a “key card deposit”. **Never reword it and never make its figures follow Settings** — the guest must read the same words they signed.
...
- **Breakfast (cards k140/k142)**: Breakfast is the **ROOM's** choice, never a guest's, and it is **one charge for the whole stay at that room's own breakfast price** — a price the desk types per room in Settings, blank until they do. It is folded into the room rate, so a room with breakfast prints on the bill as ONE line, `Room 6 · Breakfast`, and never as a row of its own. The per-day `BreakfastRecorder` is **retired and not rendered**; the ₱150 × beds idea was dropped when the owner asked for a price per room instead, so **no bed count is used anywhere**. Legacy `breakfast_records` (per-day, per-person) still price old bookings, and the bill prints those rows for a venue booking only. **`calculatePricing` charges it ONLY when it is told — `breakfastIncluded: true` — so every re-pricer must hand that flag over from the booking.** `createManualBooking` (`hooks/useBookings.ts`) did not, and stored every new booking owing the **room alone** while ticking `breakfast_included` on the row: GRF-2026-09-0014 held ₱1,800 against a ₱2,400 stay, so the printed bill showed Sub-Total ₱2,400 with an **Amount Due of ₱1,800** copied from that short balance — the same failure shape as the short-stay rule above, where a re-pricer not handed the flag prices a different stay than the one stored. `recomputeBalance`, `bookingPricing` and the Earnings report all pass it correctly, which is why the ₱600 reappeared the moment the guest was checked in. **The guest portal is still on the retired path**: it quotes breakfast as the old `breakfastEnabled` figure (₱150 × guests × days) while saving `breakfast_included: true`, so its quoted total can differ from the room-priced bill; changing it awaits the owner's say-so.

- **Short stays (the printed rate board)**: `calculatePricing` takes `shortStayHours` (3/6/12/22 — 22 is still accepted because a booking saved while the form still offered it must keep pricing correctly, but nothing sells it any more: 22 hours IS the room's normal price, so it is an ordinary overnight booking booked from a date range). A short stay is **ONE charge**, never nights × rate: the price is the room's own figure for those hours (`hour3_price` / `hour6_price` / `hour12_price`), and for 22 hours it is the room's normal single price — that 22-hour column **is** the price the app has always charged. `stayQuantity` is 1 and `stayUnit` becomes `3 HOURS`, so the bill and the statement print hours instead of nights. **Every reader that re-prices a booking MUST pass `shortStayHours: <booking>.stay_hours`** — `recomputeBalance` (`bookingBalance.ts`), `bookingPricing` (`statement.ts`), the Earnings report (`useAnalyticsCalculations.ts`) and `createManualBooking` (`useBookings.ts`). Those four skipped it at first, so a 3-hour stay on Room 8 was ₱550 in the booking form and **₱850 — a whole night —** in the quick view, at check-out (which then WROTE ₱850 into the booking), on the printed bill and in the Earnings report (the owner's S1 ruling, 2026-09). Only the two `calculatePricing` calls that price an **extension** (`CalendarTab` / `ExtendStayModal`) leave it out on purpose: a stay extended past its day becomes a normal night. `chargeableEarlyHours` returns **0** whenever `stay_hours` is set, and `ExtendStayModal` skips the early/late computation for it: a three-hour guest arriving at 10am is not four hours early for a 2pm check-in, and without that gate they would be charged a whole extra night at check-out.
